...
fn discover_cpu_freqs() -> Vec<FreqSensor> {
    ...
    if let Ok(entries) = fs::read_dir("/sys/devices/system/cpu") {
        for e in entries.flatten() {
            ...
            let idx = match name.trim_start_matches("cpu").parse::<usize>() {
                Ok(v) => v,
                Err(_) => continue,
            };
...
fn read_freq_khz(path: &PathBuf) -> Option<f64> {
            ...
            if let Ok(files) = fs::read_dir(&base) {
                for f in files.flatten() {
                    ...
                    if fname.starts_with("temp") && fname.ends_with("_input") {
                        ...
                        if let Ok(lbl) = fs::read_to_string(label_path) {
                            let l = lbl.trim();
                            if !l.is_empty() {
                                label = l.to_string();
                            }
...
fn read_temp_c(path: &PathBuf) -> Option<f64> {
    let mut s = String::new();
    fs::File::open(path).ok()?.read_to_string(&mut s).ok()?;
...
fn classify(raw: &str) -> (String, String, f64, f64) {
    ...
    if r.contains("coretemp") || r.contains("k10temp") || r.contains("zen") || r.contains("cpu") {
        return ("cpu".into(), "CPU".into(), 90.0, 100.0);
    }
    if r.contains("amdgpu") {
        return ("gpu".into(), "GPU (amdgpu)".into(), 85.0, 95.0);
    }
    if r.contains("nvidia") || r.contains("gpu") {
        return ("gpu".into(), "GPU (nvidia)".into(), 85.0, 95.0);
    }
    if r.contains("nvme") {
        return ("nvme".into(), "NVMe SSD".into(), 70.0, 80.0);
    }
    if r.contains("spd") {
        return ("ramspd".into(), "Memory (SPD Hub)".into(), 70.0, 85.0);
    }
    if r.contains("iwlwifi") {
        return (
            "wifi".into(),
            "Wi‑Fi Controller (iwlwifi)".into(),
            80.0,
            90.0,
        );
    }
    if r.contains("r8169") {
        return (
            "eth".into(),
            "Ethernet Controller (r8169)".into(),
            80.0,
            90.0,
        );
    }
    if r.contains("igc") {
        return ("eth".into(), "Ethernet Controller (igc)".into(), 80.0, 90.0);
    }
    if r.contains("e1000") {
        return (
            "eth".into(),
            "Ethernet Controller (e1000)".into(),
            80.0,
            90.0,
        );
    }
    if r.contains("r8125") {
        return (
            "eth".into(),
            "Ethernet Controller (r8125)".into(),
            80.0,
            90.0,
        );
    }
    if r.contains("acpitz") {
        return (
            "acpi".into(),
            "System Temperature (acpitz)".into(),
            80.0,
            95.0,
        );
    }
    if r.contains("pch") || r.contains("isa") {
        return ("chipset".into(), "Chipset".into(), 85.0, 95.0);
    }
...
fn nice_label(group: &str, raw: &str) -> String {
    ...
    match group {
        "CPU" => {
            if ll.contains("package") {
                "CPU (Package)".into()
            } else if ll.contains("tctl") || ll.contains("tdie") {
                "CPU (Composite)".into()
            } else if ll.starts_with("core ") {
...
fn build_groups() -> Vec<SensorGroup> {
    ...
    for g in map.values_mut() {
        ...
        for it in &mut g.items {
            ...
            let is_composite =
                n.contains("composite") || n.contains("package") || n.contains("core)");
            if !showed
                && (is_composite
                    || g.display.contains("Wi‑Fi Controller")
                    || g.display.contains("Ethernet Controller")
                    || g.display.contains("System Temperature"))
            {
                it.visible = true;
                showed = true;
            }
        ...
        if !showed {
            if let Some(first) = g.items.first_mut() {
                first.visible = true;
            }
        }
    }
        ...
        if g.display.starts_with("CPU") {
            g.items.sort_by(|a, b| {
                fn key(name: &str) -> (u8, i32, String) {
                    ...
                    let mut tier: u8 = 3;
                    if ln.contains("package") || ln.contains("composite") {
                        tier = 0;
                    } else if ln.contains("cpu (core ") {
                        tier = 1;
                    }
...
impl App {
    fn new(capacity_secs: usize, sample_hz: f64) -> Self {
        ...
        let temp_series = HWMON_SENSORS
            .iter()
            .map(|_| RollingSeries::new(capacity_secs))
            .collect::<Vec<_>>();
        let freq_series = FREQ_SENSORS
            .iter()
            .map(|_| RollingSeries::new(capacity_secs))
            .collect::<Vec<_>>();
        ...
        for i in 0..FREQ_SENSORS.len() {
            freq_colors.push(pal[i % pal.len()]);
        }
    ...
    fn sample(&mut self) {
        ...
        // CPU / RAM %
        let avg_cpu: f32 = self.sys.cpus().iter().map(|c| c.cpu_usage()).sum::<f32>()
        ...
        #[cfg(feature = "nvidia")]
        {
            if let Some(nv) = &self.nv {
                if let Some((gpu_pct, vram_pct, temp_c)) = nvgpu::first_gpu_metrics(nv) {
                    ...
                    if let Some(idx) = self.gpu_temp_idx {
                        self.temp_series[idx].push(self.seconds, temp_c);
                    }
                } else {
        ...
        for (i, fsens) in FREQ_SENSORS.iter().enumerate() {
            if let Some(khz) = read_freq_khz(&fsens.path) {
                self.freq_series[i].push(self.seconds, khz);
            }
        }
        // Temperatures
        for (i, ts) in HWMON_SENSORS.iter().enumerate() {
            if let Some(t) = read_temp_c(&ts.path) {
                self.temp_series[i].push(self.seconds, t);
            }
        }
    }
}
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.label(format!(
                    "Samples: {}",
                    (self.seconds / self.sample_period.as_secs_f64()) as usize
                ));
                ui.separator();
                ui.label(format!(
                    "CPU: {:.0}%",
                    self.cpu_util.last_y().unwrap_or(0.0)
                ));
                ui.separator();
                ui.label(format!(
                    "RAM: {:.0}%",
                    self.ram_util.last_y().unwrap_or(0.0)
                ));
            });
        });
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let (auto_xmin, auto_xmax) = if self.seconds > self.display_window_secs {
            ...
            util_plot.show(ui, |plot_ui| {
                ...
                let ticks = 4;
                let step = (ymax - ymin) / (ticks as f64);
                let mut v = ymin;
                while v <= ymax + 1e-6 {
                    plot_ui.text(
                        Text::new([xmin, v].into(), format!("{:.0}%", v))
                ...
                plot_ui.line(
                    Line::new(self.cpu_util.points_after(xmin))
                        .name("CPU %")
                        .color(theme_color("cpu")),
                );
                plot_ui.line(
                    Line::new(self.gpu_util.points_after(xmin))
                        .name("GPU %")
                        .color(theme_color("gpu")),
                );
                plot_ui.line(
                    Line::new(self.ram_util.points_after(xmin))
                        .name("RAM %")
                        .color(theme_color("ramspd")),
                );
                plot_ui.line(
                    Line::new(self.vram_util.points_after(xmin))
                        .name("VRAM %")
                ...
                while v2 <= ymax + 1e-6 {
                    plot_ui.text(
                        Text::new([xmax, v2].into(), format!("{:.0}%", v2))
            ...
            temp_plot.show(ui, |plot_ui| {
                ...
                for g in &self.groups {
                    ...
                    for it in &g.items {
                        ...
                        if let Some((a, b)) = self.temp_series[it.idx].min_max_y(xmin, xmax) {
                            if a < mn {
                                mn = a;
                            }
                            if b > mx {
                ...
                if !mn.is_finite() || !mx.is_finite() || (mx - mn).abs() < 1e-6 {
                    mn = 0.0;
                    mx = 120.0;
                }
                let pad = ((mx - mn) * 0.1).max(2.0);
                mn = (mn - pad).max(0.0);
                mx = (mx + pad).min(130.0);
                ...
                for g in &self.groups {
                    ...
                    for it in &g.items {
                        ...
                        plot_ui.line(
                            Line::new(pts)
                                .name(format!("{}: {}", g.display, it.name))
                                .color(it.color),
                        );
                    }
                }
                let ticks = 4;
                let step = (mx - mn) / (ticks as f64);
                let mut v = mn;
                while v <= mx + 1e-6 {
                    plot_ui.text(
                        Text::new([xmax, v].into(), format!("{:.0}", v))
                            .anchor(Align2::RIGHT_CENTER),
                    );
                    v += step;
                }
            ...
            freq_plot.show(ui, |plot_ui| {
                ...
                for (i, series) in self.freq_series.iter().enumerate() {
                    if !self.freq_visible.get(i).copied().unwrap_or(false) {
                        continue;
                    }
                    if let Some((a, b)) = series.min_max_y(xmin, xmax) {
                        let ag = a / 1_000_000.0;
                        let bg = b / 1_000_000.0;
                        if ag < mn {
                            mn = ag;
                        }
                        if bg > mx {
                ...
                #[cfg(feature = "nvidia")]
                {
                    if self.gpu_freq_graphics_vis {
                        if let Some((a, b)) = self.gpu_clk_graphics.min_max_y(xmin, xmax) {
                            let ag = a / 1000.0;
                            let bg = b / 1000.0;
                            if ag < mn {
                                mn = ag;
                    ...
                    if self.gpu_freq_sm_vis {
                        if let Some((a, b)) = self.gpu_clk_sm.min_max_y(xmin, xmax) {
                            let ag = a / 1000.0;
                            let bg = b / 1000.0;
                            if ag < mn {
                                mn = ag;
                    ...
                    if self.gpu_freq_mem_vis {
                        if let Some((a, b)) = self.gpu_clk_mem.min_max_y(xmin, xmax) {
                            let ag = (a / 1000.0) * if self.gpu_mem_effective { 2.0 } else { 1.0 };
                            let bg = (b / 1000.0) * if self.gpu_mem_effective { 2.0 } else { 1.0 };
                            if ag < mn {
                                mn = ag;
                            }
                            if bg > mx {
                    ...
                    if self.gpu_freq_video_vis {
                        if let Some((a, b)) = self.gpu_clk_video.min_max_y(xmin, xmax) {
                            let ag = a / 1000.0;
                            let bg = b / 1000.0;
                            if ag < mn {
                                mn = ag;
                ...
                for (i, series) in self.freq_series.iter().enumerate() {
                    ...
                    let name = format!(
                        "CPU Core {}",
                        FREQ_SENSORS.get(i).map(|s| s.core).unwrap_or(i)
                    );
                    let pts = series.points_after_scaled(xmin, 1_000_000.0);
                    plot_ui.line(
                        Line::new(pts)
                            .name(name)
                            .color(self.freq_colors[i % self.freq_colors.len()]),
                    );
                }
                #[cfg(feature = "nvidia")]
                {
                    if self.gpu_freq_graphics_vis {
                        let pts = self.gpu_clk_graphics.points_after_scaled(xmin, 1000.0);
                        plot_ui.line(Line::new(pts).name("GPU Graphics"));
                    }
                    if self.gpu_freq_sm_vis {
                        let pts = self.gpu_clk_sm.points_after_scaled(xmin, 1000.0);
                        plot_ui.line(Line::new(pts).name("GPU SM"));
                    }
                    if self.gpu_freq_mem_vis {
                        let div = 1000.0 / if self.gpu_mem_effective { 2.0 } else { 1.0 };
                        let pts = self.gpu_clk_mem.points_after_scaled(xmin, div);
                        let label = if self.gpu_mem_effective {
                    ...
                    if self.gpu_freq_video_vis {
                        let pts = self.gpu_clk_video.points_after_scaled(xmin, 1000.0);
                        plot_ui.line(Line::new(pts).name("GPU Video"));
                    }
                }
                let ticks = 4;
                let step = (mx - mn) / (ticks as f64);
                ...
                while v <= mx + 1e-6 {
                    plot_ui.text(
                        Text::new([xmax, v].into(), format!("{:.2} GHz", v))
                            .anchor(Align2::RIGHT_CENTER),
                    );
                    v += step;
                }
                        ...
                        .show(ui, |ui| {
                            for g in &mut self.groups {
                                if g.display.starts_with("CPU") {
                                            ...
                                            ui.with_layout(
                                                ...
                                                |ui| {
                                                    ...
                                                    ui.allocate_ui_with_layout(
                                                        egui::vec2(left_px, 0.0),
                                                        layout,
                                                        |ui| {
                                                            ui.label(
                                                                RichText::new("Core temperatures")
                                                                    .strong(),
                                                            );
                                                            for it in &mut g.items {
                                                                ui.checkbox(
                                                    ...
                                                    ui.allocate_ui_with_layout(
                                                        egui::vec2(right_px, 0.0),
                                                        layout,
                                                        |ui| {
                                                            ui.label(
                                                                RichText::new("Core frequencies")
                                                                    .strong(),
                                                            );
                                                            ui.horizontal(|ui| {
                                                            ...
                                                            for (i, fs) in
                                                                FREQ_SENSORS.iter().enumerate()
                                                            {
                                                                let mut vis = self.freq_visible[i];
                                                                let label =
                                                                    format!("CPU Core {}", fs.core);
                                            ...
                                            ui.with_layout(
                                                ...
                                                |ui| {
                                                    ...
                                                    ui.allocate_ui_with_layout(
                                                        ...
                                                        |ui| {
                                                            ui.label(
                                                                RichText::new("Frequencies")
                                                                    .strong(),
                                                            );
                                                            #[cfg(feature = "nvidia")]
                                                            {
                                                                ui.checkbox(
                                                                    &mut self.gpu_freq_graphics_vis,
                                                                    "GPU Graphics",
                                                                );
                                                                ui.checkbox(
                                                                    &mut self.gpu_freq_sm_vis,
                                                                    "GPU SM",
                                                                );
                                                                ui.checkbox(
                                                                ...
                                                                ui.checkbox(
                                                                    &mut self.gpu_freq_mem_vis,
                                                                    "GPU Memory",
                                                                );
                                                                ui.checkbox(
                                                                    &mut self.gpu_freq_video_vis,
                                                                    "GPU Video",
                                                                );
                                                            }
                                                        },
                                    ...
                                    egui::CollapsingHeader::new(g.display.clone())
                                        .id_source(format!("grp_other_{}", g.display))
                                        .default_open(false)
                                        .show(ui, |ui| {
                                            for it in &mut g.items {
                                                ui.checkbox(&mut it.visible, &it.name);
                                            }
                                        });
                                }
                                if cols > 1 {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            for g in &self.groups {
                ...
                for it in &g.items {
                    ...
                    let hot = self.temp_series[it.idx]
                        .last_y()
                        .map(|y| y >= g.hot)
                        .unwrap_or(false);
                    let warn = self.temp_series[it.idx]
                        .last_y()
                        .map(|y| y >= g.warn)
                        .unwrap_or(false);
                    if hot {
