...
impl App {
    ...
    fn series_label(series: &Series) -> String {
        format!(
            "{} ({})",
            series.descriptor.display_name, series.entity_id.0
        )
    }
}
...
impl eframe::App for App {
    fn update(&mut self, context: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        context.request_repaint_after(self.sample_period.saturating_sub(self.last_poll.elapsed()));
        ...
        egui::TopBottomPanel::top("top").show(context, |ui| {
            ui.horizontal_wrapped(|ui| {
                ...
                ui.label(format!(
                    "RAM: {:.0}%",
                    self.latest_metric("system.memory.utilization")
                        .unwrap_or(0.0)
                ));
            });
        });
        ...
        egui::CentralPanel::default().show(context, |ui| {
            ...
            ui.label(
                RichText::new(
                    "Unavailable capabilities are omitted; missing observations appear as gaps.",
                )
                .small(),
            );
        });
...
fn padded_bounds(minimum: f64, maximum: f64, floor: f64, ceiling: f64) -> (f64, f64) {
    ...
    (
        (minimum - padding).max(floor),
        (maximum + padding).min(ceiling),
    )
}
...
fn series_color(metric_id: &str, index: usize) -> Color32 {
    ...
    let lighten =
        |component: u8| (component as f32 + (255.0 - component as f32) * factor).min(255.0) as u8;
