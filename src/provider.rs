...
// These arguments mirror the independently typed descriptor dimensions; grouping them would
// obscure the mapping at each provider declaration.
    ...
    fn observe(&mut self, observation_time: MonotonicTimestamp) -> Vec<ProviderResult> {
        ...
        for identity in &self.devices {
            ...
            let occupancy = device.memory_info().ok().and_then(|memory| {
                (memory.total > 0)
                    .then(|| MetricValue::F64(memory.used as f64 * 100.0 / memory.total as f64))
            });
            results.push(observation_owned(
